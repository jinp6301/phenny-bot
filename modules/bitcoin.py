...
def ltc(phenny, input):
   ...
   useasternDatetime = utcDatetime.astimezone(useastern)
   if high:
      phenny.reply("LTC price at %s - High: US$%s Low: US$%s Average US$%s" % (useasternDatetime.strftime('%Y-%m-%d %H:%M:%S %z'), high, low, avg))
...
def btc(phenny, input): 
   ...
   useasternDatetime = utcDatetime.astimezone(useastern)
   if high: 
      phenny.reply("Mt Gox price at %s - High: %s Low: %s Average %s" % (useasternDatetime.strftime('%Y-%m-%d %H:%M:%S %z'), high, low, avg))
   else: phenny.reply("No data returned.")
btc.commands = ['btc']
btc.priority = 'high'
btc.example = '.btc'
